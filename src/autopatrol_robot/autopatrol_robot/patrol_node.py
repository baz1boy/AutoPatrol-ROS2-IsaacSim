...
class PatrolNode(BasicNavigator):
    ...
    def __init__(self):
        ...
        self.img_sub_ = self.create_subscription(Image, '/pabot_cam/rgb/image_raw', self.img_callback, 1)
    ...
    def record_img(self):
        if self.latest_img_ is not None:
            ...
            success = cv2.imwrite(full_path, cv_image)
            if success:
                self.get_logger().info(f'图片已保存: {full_path}')
            else:
                self.get_logger().warn(f'图片保存失败: {full_path}')
